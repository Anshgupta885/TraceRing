      ...
      <div className="card mb-6" style={{ padding: '1.25rem' }}>
        ...
        <form onSubmit={handleSubmit} style={{ marginTop: '1rem', display: 'grid', gap: '0.75rem' }}>
          ...
          <div className="password-field-wrapper">
            <input required placeholder="Password" type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className="input" aria-describedby="signup-toggle-password" />
            <button id="signup-toggle-password" type="button" aria-label={showPassword ? 'Hide password' : 'Show password'} className="password-toggle" onClick={() => setShowPassword(s => !s)}>
              {showPassword ? (
